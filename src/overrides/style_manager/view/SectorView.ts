...
export default class SectorView extends View<Sector> {
  ...
  updateOpen() {
    ...
    $el[isOpen ? 'addClass' : 'removeClass']('gjs-open');
    if (isOpen) {
      this.el.children[0].firstElementChild?.classList.replace('fa-caret-right', 'fa-caret-down');
    } else {
      this.el.children[0].firstElementChild?.classList.replace('fa-caret-down', 'fa-caret-right');
    }
