...
export default class LayersView extends View<Layer> {
  ...
  addToCollection(model: Layer, fragmentEl: DocumentFragment | null, index?: number) {
    ...
    const labelEl = view.getLabelEl();
    if (labelEl) {
      const layerIndex = index ?? this.items.length;
      labelEl.innerHTML = `Layer ${layerIndex + 1}`;
    }
    ...
    if (children.length > insertIndex) {
      children.eq(insertIndex).before(rendered); // Insert at correct position
    } else {
      $el.append(rendered); // Append at end
    }

    // Select the first layer added
    if (index != undefined) {
