...
import { CommandObject } from '../../../commands/view/CommandAbstract';
...
export default {
  ...
  initResize(elem: HTMLElement) {
    ...
    if (model && resizable) {
      ...
      if (!(canvas.getElement() instanceof HTMLElement)) {
      ...
      let canvasFrameElementWidthOffset = (canvasWidth - canvasFrameElementWidth) / 2;
      if (canvasFrameElementWidth > canvasWidth) canvasFrameElementWidthOffset = 0;
      ...
      const options: ResizerOptions = {
        ...
        onStart(e: Event, opts: any = {}) {
          ...
          startInfo.mouseX = (e as PointerEvent).clientX;
          startInfo.componentLeft = parseFloat(el.offsetLeft); //modelStyle.left can contain 'nan', so don't use
          startInfo.mouseLeftOffset =
            startInfo.mouseX - canvasLeftOffset - startInfo.componentLeft - canvasFrameElementWidthOffset;
          startInfo.mouseY = (e as PointerEvent).clientY;
          startInfo.componentTop = parseFloat(el.offsetTop);
          startInfo.mouseTopOffset = startInfo.mouseY - canvasTopOffset - startInfo.componentTop;
        ...
        onEnd(e: Event, opts: any) {
          ...
          editor.trigger('component:resize');
          editor.trigger('component:resize:end', opts.resizer.startDim, opts.resizer.rectDim); // this event is not a native GrapesJS event, it was added for CCIDE
          ...
          if (!onlyHeight) {
            ...
            if (leftHandler) {
              let handlerRightLimit =
                startInfo.mouseX + startInfo.width - minComponentWidth - startInfo.mouseLeftOffset;
              if (width > minComponentWidth) {
                let resizerCurrentXPosition = options.resizer.currentPos['x'];
                let resizerLeftPos =
                  resizerCurrentXPosition < handlerRightLimit ? resizerCurrentXPosition : handlerRightLimit;
                const componentleftPos =
                  resizerLeftPos - canvasLeftOffset - startInfo.mouseLeftOffset - canvasFrameElementWidthOffset;
                style['left'] = `${componentleftPos}${unitWidth}`;
              } else {
                const componentleftPos = handlerRightLimit - canvasLeftOffset - canvasFrameElementWidthOffset;
                style['left'] = `${componentleftPos}${unitWidth}`;
              }
            }
            ...
            // limit checks
            if (rightHandler) {
              let rightLimitReached = canvasFrameElementWidth - rect.w - el.offsetLeft <= 0 ? true : false;
          ...
          if (!onlyWidth) {
            ...
            if (topHandler) {
              let handlerBottomLimit =
                startInfo.mouseY - startInfo.mouseTopOffset + startInfo.height - minComponentHeight;
              if (rect.h > minComponentHeight) {
                let resizerCurrentYPosition = options.resizer.currentPos['y'];
                let resizerTopPos =
                  resizerCurrentYPosition < handlerBottomLimit ? resizerCurrentYPosition : handlerBottomLimit;
            ...
            // limit checks
            if (bottomHandler) {
              let bottomLimitReached = canvasFrameElementHeight - rect.h - el.offsetTop <= 0 ? true : false;
