...
export default class InputNumber extends Input {
  ...
  checkInvalidCharacters(val: any, unit: string) {
  ...
  getUnitEl() {
    if (!this.unitEl) {
      ...
      if (units.length) {
        const options = [`<option value="" disabled hidden>${units[0]}</option>`];
  ...
  moveIncrement(ev: MouseEvent) {
    ...
    model.set({ value, unit }, { avoidStore: 1, fromInput: 1 });
    return false;
  }
